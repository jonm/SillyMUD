...
void do_backstab(struct char_data *ch, char *argument,
  ...
  if (MOUNTED(ch)) {
    send_to_char
      ("How can you surprise someone with all the racket that beast makes?\n\r",
       ch);
    return;
  }
  ...
  if (ch->skills && ch->skills[SKILL_SNEAK].learned) {
    if (IS_SET(ch->specials.act, ACT_AGGRESSIVE) ||
      ...
      if (percent > ch->skills[SKILL_SNEAK].learned) {  /* failed sneak */
        ...
        if (IS_AFFECTED(ch, AFF_FLYING))
          act("You quietly glide up behind $N.", FALSE, ch, 0, victim,
              TO_CHAR);
        else
          act("You quietly approach $N from behind.", FALSE, ch, 0, victim,
              TO_CHAR);
      }
    }
...
void do_flee(struct char_data *ch, char *argument,
  ...
  if (IS_PC(ch) || IS_SET(ch->specials.act, ACT_POLYSELF)) {
    ...
    if (IS_AFFECTED2(ch, AFF2_BERSERK)) {
      send_to_char
        ("Flee now?  But your opponent aint dead!  Killl, kill, kill.\n\r",
         ch);
      return;
    }
          ...
          if (CAN_GO(ch, attempt)
            ...
            if (RIDDEN(ch)) {
              if ((die = move_one(RIDDEN(ch), attempt)) == 1) {
                ...
                if (!die)
                  act("$n tries to flee, but is too exhausted!", TRUE, ch, 0,
                      0, TO_ROOM);
                break;
                /* return; */
              }
      ...
      for (f = 0; f < ((ch->specials.flee) - tries); f++) {
        for (i = 0; i < 6; i++) {
          ...
          if (CAN_GO(ch, attempt)
            ...
            send_to_char
              ("There is NO WAY IN HELL you'd consider going that way!\n\r",
               ch);
...
void do_wimp(struct char_data *ch, char *argument,
  ...
  if (((!IS_NPC(ch)) && (IS_SET(ch->specials.act, PLR_WIMPY)))
      || ((IS_NPC(ch)) && (IS_SET(ch->specials.act, ACT_WIMPY)))) {
    if (strlen(buf) != 0) {
      pct = atoi(buf);
      if (pct < 1) {
