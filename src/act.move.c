...
*/
  ...
  if (!exit_ok(exitp, NULL)) {
    ...
    send_to_char
      ("Sorry, you'd either have to be flying or climbing to get there!\n\r",
       ch);
    return (FALSE);
...
int raw_move(struct char_data *ch, int dir) {
  ...
  if (special(ch, move_dir_string[dir], "")) /* Check for special routines(North is 1) */
    return (FALSE);
  ...
  if ((from_here->sector_type == SECT_TREE) ||
    ...
    if (!IS_AFFECTED(ch, AFF_TREE_TRAVEL)) {
      send_to_char
        ("You would have to be able to walk through trees to go there!\n\r",
         ch);
      return (FALSE);
    }
    if (MOUNTED(ch)) {
      if (!IS_AFFECTED(MOUNTED(ch), AFF_TREE_TRAVEL)) {
        send_to_char
          ("Your mount would have to be able to walk through trees to go there!\n\r",
           ch);
        return (FALSE);
      }
    }
...
void move_to_dir(struct char_data *ch, int dir) {
  ...
  if (ch->attackers > 1) {
    send_to_char
      ("There's too many things trying to hit you, no place to flee!\n\r", ch);
    return;
  }

  if (ch->specials.fighting) {
    send_to_char
      ("Hey, maybe you should try fleeing during combat situations!\n\r", ch);
    return;
  }
        ...
        if (dir < 4) {
          ...
          if (total == 1) {
            if (MOUNTED(ch)) {
              ...
              SPRINTF(tmp, "%s %s from below.", PERS(ch, tmp_ch), how);
            }
          }
          else {
            SPRINTF(tmp, "%s %s from below.", PERS(ch, tmp_ch), how);
          ...
          if (total == 1) {
            if (MOUNTED(ch)) {
              ...
              SPRINTF(tmp, "%s %s from above", PERS(ch, tmp_ch), how);
            }
          }
          else {
            SPRINTF(tmp, "%s %s from above.", PERS(ch, tmp_ch), how);
