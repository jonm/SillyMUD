    ...
    if (GET_RACE(tmp_victim) == RACE_ORC && CAN_SEE(ch, tmp_victim) &&
      ...
      if ((GET_HIT(ch) / GET_MAX_HIT(ch) > 0.25) &&
        ...
        if (!saves_spell(ch, SAVING_SPELL)) {
          send_to_char
            ("Your vision turns red as bloodlust overcomes you.!\n\r", ch);
          ...
          send_to_char
            ("You manage to supress the urge to smash this puny orc to pieces.\n\r",
             ch);
        }
      }
      else {
