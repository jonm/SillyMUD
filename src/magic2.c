  ...
  if (is_undead(victim)) {
    ...
    if (diff <= -4) {           /* The puny girly girly clerics just don't cut it */
      ...
      if ((number(1, 100)) <= chance) {
        ...
        act("$N laughs at $n's incompetance.", TRUE, ch, 0, victim,
            TO_NOTVICT);
...
void spell_holy_word(byte level, struct char_data *ch,
  ...
  if (IS_GOOD(ch)) {            /* If the caster is GOOD */
    send_to_char
      ("You feel righteous fury fill your body as you declare your holy word!\n\r",
       ch);
    act
      ("$n bellows a holy word!  You stagger from the blast and shield your eyes!",
       TRUE, ch, 0, victim, TO_ROOM);
  }

  if (IS_EVIL(ch)) {            /* If the caster is EVIL */
    send_to_char
      ("You feel demonic power fill your soul as you declare your unholy word!\n\r",
       ch);
    act("$n screams an unholy word!  Waves of nausea make you weary and sick!",
        TRUE, ch, 0, victim, TO_ROOM);
  }

  if (!(IS_GOOD(ch)) && !(IS_EVIL(ch))) {       /* Caster must be neutral! */
    send_to_char
      ("You feel ecstasy in becoming as one with absolute emptiness.\n\r", ch);
...
/* gets rid of infravision, invisibility, detect, etc */
  ...
  if (affected_by_spell(victim, SPELL_FIRESHIELD)) {
    if (yes || !saves_spell(victim, SAVING_SPELL)) {
      ...
      send_to_char("You don't feel so fiery anymore.\n\r", victim);
      act("The fiery aura around $n's body fades.", FALSE, victim, 0, 0,
          TO_ROOM);
    }
    /*
     *  aggressive Act.
...
void spell_web(byte level, struct char_data *ch,
  ...
  case RACE_GHOST:
    ...
    act("Hah, $n casted web on you, what a flake.", FALSE, ch, 0, victim,
        TO_VICT);
...
void spell_conjure_elemental(byte UNUSED(level), struct char_data *ch,
  ...
  if (too_many_followers(ch)) {
    act
      ("$N says 'No way I'm hanging with that crowd, but thanks for the lift'",
       TRUE, ch, 0, victim, TO_ROOM);
    act
      ("$N refuses to hang out with crowd of your size, but thanks you for the free ride.",
       TRUE, ch, 0, victim, TO_CHAR);
  }
  else {
