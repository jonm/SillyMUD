...
void affect_total(struct char_data *ch) {
  ...
  for (af = ch->affected; af; af = af->next)
    if (af->type != SPELL_FLY)
      affect_modify(ch, af->location, (int)af->modifier, af->bitvector, FALSE);
...
void affect_remove(struct char_data *ch, struct affected_type *af) {
  ...
  if (ch->affected == af) {
    ...
    if (hjp->next != af) {
      log_msg
        ("Could not locate affected_type in ch->affected. (handler.c, affect_remove)");
      return;
    }
...
void equip_char(struct char_data *ch, struct obj_data *obj, int pos) {
  ...
  if (j < -5) {
    ...
    act
      ("$p almost seems to say 'You're pretty puny.  I don't want to be seen with you!\n",
       0, ch, obj, 0, TO_CHAR);
    act("$p falls to the floor", 0, ch, obj, 0, TO_CHAR);
    act("$p removes itself, and falls to the floor", 0, ch, obj, 0, TO_ROOM);
    obj_to_room(obj, ch->in_room);
  ...
  if (item_align_clash(ch, obj)) {
    if (ch->in_room != NOWHERE) {
      ...
      log_msg("ch->in_room = NOWHERE when equipping char.");
      assert(0);
    }
  }
...
void extract_obj(struct obj_data *obj) {
  ...
  if (object_list == obj)       /* head of list */
    ...
    if (temp1) {
      ...
      log_msg("Couldn't find object in object list.");
      assert(0);
    }
  }
...
void extract_char_smarter(struct char_data *ch, int save_room) {
  ...
  if (ch->in_room == NOWHERE) {
    log_msg("NOWHERE extracting char. (handler.c, extract_char)");
    /*
