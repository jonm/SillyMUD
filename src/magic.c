...
void spell_armor(byte level, struct char_data *ch,
  ...
  if (IS_NEUTRAL(ch) ||
      (IS_GOOD(ch) && !IS_EVIL(victim)) || (IS_EVIL(ch) && !IS_GOOD(victim))) {
    ...
    if (!affected_by_spell(victim, SPELL_ARMOR)) {
      ...
      if (ch != victim)
        act("$n encases $N in a mystical armor.", TRUE, ch, 0, victim,
            TO_NOTVICT);
      else
        act("$n is encased by a mystical force.", TRUE, ch, 0, victim,
            TO_NOTVICT);
    }
    else {
...
void spell_astral_walk(byte UNUSED(level), struct char_data *ch,
                       struct char_data *UNUSED(victim),
                       struct obj_data *UNUSED(obj)) {
  int entrance;
...
void spell_teleport(byte UNUSED(level), struct char_data *ch,
                    struct char_data *victim, struct obj_data *UNUSED(obj)) {
  int to_room, try = 0;
    ...
    if ((GET_POS(victim) != POSITION_FIGHTING) &&
      ...
      if (IS_EVIL(ch)) {
        if (ch != victim)
          ...
          act("$n receives an unholy blessing.", TRUE, ch, 0, victim,
              TO_NOTVICT);
        send_to_char("You feel wicked.\n\r", victim);
      }
      else {
...
void spell_control_weather(byte UNUSED(level), struct char_data *UNUSED(ch),
                           struct char_data *UNUSED(victim),
                           struct obj_data *UNUSED(obj)) {
...
void spell_curse(byte level, struct char_data *ch,
  ...
  if (obj) {
    ...
    if (ch != victim) {
      ...
      act("$n is sorrounded in a malignant aura!", TRUE, ch, 0, 0, TO_NOTVICT);
...
void spell_detect_poison(byte UNUSED(level), struct char_data *ch,
                         struct char_data *victim, struct obj_data *obj) {
  assert(ch && (victim || obj));
...
void spell_invisibility(byte UNUSED(level), struct char_data *ch,
                        struct char_data *victim, struct obj_data *obj) {
  struct affected_type af;
  ...
  if (victim) {
    ...
    if (is_immune(victim, IMM_POISON) || (!IS_SET(ch->specials.act, ACT_DEADLY)
      ...
      act("$n tried to poison $N, but $E ignores it!",
          TRUE, ch, 0, victim, TO_NOTVICT);
      act("$n ignores your poison!", TRUE, victim, 0, ch, TO_VICT);
      if (!victim->specials.fighting)
        set_fighting(victim, ch);
      return;
    }
    ...
    if (IS_SET(ch->specials.act, ACT_DEADLY) && ch != victim) {
      ...
      if (is_resist(victim, IMM_POISON)) {
        ...
        if (!imp_save_spell(victim, SAVING_PARA, 0)) {
          ...
          act("$n tries to poison $N, but $E resists it!",
              TRUE, ch, 0, victim, TO_NOTVICT);
          act("$n resists your deadly poison!", TRUE, victim, 0, ch, TO_VICT);
          if (!victim->specials.fighting)
            set_fighting(victim, ch);
          return;
        }
      ...
      if (!imp_save_spell(victim, SAVING_PARA, 0)) {
        ...
        act("$n fills $N's veins with deadly poison!",
            TRUE, ch, 0, victim, TO_NOTVICT);
        act("You kill $N with your deadly poison!", TRUE, ch, 0, victim,
            TO_CHAR);
        ...
        act("$n fills $N's veins with deadly poison, but $E luckily resists!!",
            TRUE, ch, 0, victim, TO_NOTVICT);
        act("$N luckily resists your deadly poison!", TRUE, ch, 0, victim,
            TO_CHAR);
      ...
      if (is_resist(victim, IMM_POISON)) {
        if (imp_save_spell(victim, SAVING_PARA, 0)) {
          ...
          act("$n tries to poison $N, but $E resists it!",
              TRUE, ch, 0, victim, TO_NOTVICT);
          act("$n resists your poison!", TRUE, victim, 0, ch, TO_VICT);
          if (!victim->specials.fighting)
            set_fighting(victim, ch);
          return;
        }
      ...
      if (!imp_save_spell(victim, SAVING_PARA, 0)) {
        ...
        act("$n fills $N's veins with poison.", TRUE, ch, 0, victim,
            TO_NOTVICT);
        ...
        act("$N tried to poison you.", TRUE, victim, 0, ch, TO_CHAR);
        act("$n tries to poison $N, but it fails!",
            TRUE, ch, 0, victim, TO_NOTVICT);
        act("$n avoids your poison!", TRUE, victim, 0, ch, TO_VICT);
        if (!victim->specials.fighting)
          set_fighting(victim, ch);
        return;
      }
...
void spell_protection_from_evil(byte UNUSED(level), struct char_data *ch,
                                struct char_data *victim,
                                struct obj_data *UNUSED(obj)) {
  struct affected_type af;
  ...
  if (!affected_by_spell(victim, SPELL_PROTECT_FROM_EVIL)) {
    ...
    if (ch != victim) {
      act("$n encases $N in a faint golden glow.", TRUE, ch, 0, victim,
          TO_NOTVICT);
      act("You encase $N in a faint golden glow.", TRUE, ch, 0, victim,
          TO_CHAR);
    }
    else {
...
void spell_remove_curse(byte UNUSED(level), struct char_data *ch,
                        struct char_data *victim, struct obj_data *obj) {
...
void spell_remove_poison(byte UNUSED(level), struct char_data *ch,
                         struct char_data *victim, struct obj_data *obj) {
  ...
  if (victim) {
    if (affected_by_spell(victim, SPELL_POISON)) {
      ...
      act("You remove the poison from $N's veins.", TRUE, ch, 0, victim,
          TO_CHAR);
...
void spell_fireshield(byte level, struct char_data *UNUSED(ch),
                      struct char_data *victim, struct obj_data *UNUSED(obj)) {
  struct affected_type af;

  if (!affected_by_spell(victim, SPELL_FIRESHIELD)) {

    act("$n is surrounded by a glowing red aura.", TRUE, victim, 0, 0,
        TO_ROOM);
...
void spell_sanctuary(byte level, struct char_data *UNUSED(ch),
                     struct char_data *victim, struct obj_data *UNUSED(obj)) {
  struct affected_type af;
...
void spell_ventriloquate(byte UNUSED(level), struct char_data *UNUSED(ch),
                         struct char_data *UNUSED(victim),
                         struct obj_data *UNUSED(obj)) {
  /* Not possible!! No argument! */
}

void spell_word_of_recall(byte UNUSED(level), struct char_data *UNUSED(ch),
                          struct char_data *victim,
                          struct obj_data *UNUSED(obj)) {
...
void spell_summon(byte UNUSED(level), struct char_data *ch,
                  struct char_data *victim, struct obj_data *UNUSED(obj)) {
...
void spell_charm_person(byte UNUSED(level), struct char_data *ch,
                        struct char_data *victim,
                        struct obj_data *UNUSED(obj)) {
  struct affected_type af;
...
void spell_charm_monster(byte UNUSED(level), struct char_data *ch,
                         struct char_data *victim,
                         struct obj_data *UNUSED(obj)) {
  struct affected_type af;
