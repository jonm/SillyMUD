...
#define THE_PUB	3940
  ...
  if (open == 2 && time_info.hours == 2) {
    open = 0;
    act
      ("$n says 'We're closing for the night.\n  Thanks for coming, all, and come again!'",
  ...
  if (STREQ(cmd, "kill") ||
    ...
    if (andy == get_char_room(argm, ch->in_room)) {
      ...
      hitsleft = dice(2, 6) + 6;
      if (hitsleft < GET_HIT(ch) && get_max_level(ch) <= MAX_MORT) {
    ...
    if (scan->actflag & ACT_OVER_21 && GET_AGE(ch) < 21) {
      if (IS_IMMORTAL(ch) || has_class(ch, CLASS_THIEF)) {
        ...
        act
          ("$n tells you 'Sorry, I could lose my license if I served you alcohol.'",
           FALSE, andy, 0, ch, TO_VICT);
  ...
  if (ch == eric) {
    ...
    if (fighting) {
      ...
      act("$n throws some nasty judo on $N.", FALSE, eric, 0, target,
          TO_NOTVICT);
      act("$n throws some nasty judo on you.", FALSE, eric, 0, target,
          TO_VICT);
    ...
    case E_LONG_BEER_RUN:
      {
        ...
        if (eric->in_room != andy->in_room) {
              ...
              if (NULL == get_obj_in_list_vis(eric, *scan, eric->carrying)) {
                act
                  ("$n says 'ARGH, where's my deadbeat roommate with the rent.'",
                   FALSE, eric, 0, 0, TO_ROOM);
                act("$n says 'I need beer money.'", FALSE, eric, 0, 0,
                    TO_ROOM);
          ...
          if (*scan == NULL || 1 == dice(1, 4)) {
            act("$n says 'Catch you later, dude.'", FALSE, eric, 0, 0,
                TO_ROOM);
...
void npc_steal(struct char_data *ch, struct char_data *victim) {
  ...
  if (AWAKE(victim) && (number(0, get_max_level(ch)) == 0)) {
...
int snake(struct char_data *ch, const char *cmd, char *UNUSED(arg),
  ...
  if (ch->specials.fighting && (ch->specials.fighting->in_room == ch->in_room)) {
    ...
    cast_poison(get_max_level(ch), ch, "", SPELL_TYPE_SPELL,
      ...
      if (GET_POS(ch) == POSITION_STANDING) {
        ...
        if (IS_AFFECTED(most_hurt, AFF_TEAM_GREY)) {
          ...
          if (!check_soundproof(ch)) {
            if (number(0, 4) == 0) {
              ...
              act("$n utters the words 'judicandus dies'.",
                  1, ch, 0, 0, TO_ROOM);
              cast_cure_light(get_max_level(ch), ch, "",
          ...
          if (IS_AFFECTED(most_hurt, AFF_TEAM_AMBER)) {
            ...
            act("$n utters the words 'die punk'.", 1, ch, 0, 0, TO_ROOM);
            cast_cause_light(get_max_level(ch), ch, "",
                             SPELL_TYPE_SPELL, most_hurt, 0);
            return (TRUE);
          }
          else
    ...
    if (!affected_by_spell(tar, SPELL_PROTECT_FROM_EVIL) &&
      ...
      if (hit_or_miss(ch, tar, calc_thaco(ch))) {
        ...
        if (!IS_AFFECTED(tar, AFF_PARALYSIS)) {
          cast_paralyze(get_max_level(ch), ch, "", SPELL_TYPE_SPELL, tar, 0);
          return TRUE;
        }
      }
    }
...
int shadow(struct char_data *ch, const char *cmd, char *UNUSED(arg),
  ...
  if (ch->specials.fighting && (ch->specials.fighting->in_room == ch->in_room)) {
    act("$n touches $N!", 1, ch, 0, ch->specials.fighting, TO_NOTVICT);
    act("$n touches you!", 1, ch, 0, ch->specials.fighting, TO_VICT);
    cast_chill_touch(get_max_level(ch), ch, "", SPELL_TYPE_SPELL,
                     ch->specials.fighting, 0);
    if (ch->specials.fighting)
      cast_weakness(get_max_level(ch), ch, "", SPELL_TYPE_SPELL,
                    ch->specials.fighting, 0);
    return TRUE;
  }
  return FALSE;
...
int geyser(struct char_data *ch, const char *cmd, char *UNUSED(arg),
  ...
  if (number(0, 3) == 0) {
    ...
    cast_geyser(get_max_level(ch), ch, "", SPELL_TYPE_SPELL, 0, 0);
...
int green_slime(struct char_data *ch, const char *cmd, char *UNUSED(arg),
  ...
  for (cons = real_roomp(ch->in_room)->people; cons; cons = cons->next_in_room)
    if ((!IS_NPC(cons)) && (get_max_level(cons) < LOW_IMMORTAL))
      cast_green_slime(get_max_level(ch), ch, "", SPELL_TYPE_SPELL, cons, 0);
...
int temple_labrynth_liar(struct char_data *ch, const char *cmd, char *UNUSED(arg),
  ...
  case 10:
    do_say(ch,
           "I wouldn't kill the sentry if I were more than level 9. No way!",
           0);
    return (1);
  case 12:
...
int ringwraith(struct char_data *ch, const char *cmd, char *arg,
  ...
  if ((victim = char_holding(ring)) != NULL) {
    ...
    log_msg("a One Ring was completely disconnected!?");
    wh->ringnumber = 0;
  }
  return TRUE;
  ...
  case POSITION_STANDING:
    ...
    if (temp1) {
      if (GET_MANA(zmaster) < ZM_MANA) {
        ...
        if (!check_soundproof(ch)) {
          act
            ("$n says 'Wonderful, another loyal follower!' and grins maniacly.",
             FALSE, zmaster, 0, 0, TO_ROOM);
...
int bank(struct char_data *ch, const char *cmd, char *arg,
  ...
  if (GET_BANK(ch) > get_max_level(ch) * 40000 && get_max_level(ch) < 40) {
    send_to_char
      ("I'm sorry, but we can no longer hold more than 40000 coins per level.\n\r",
       ch);
    GET_GOLD(ch) += GET_BANK(ch) - get_max_level(ch) * 40000;
    GET_BANK(ch) = get_max_level(ch) * 40000;
  }
  ...
  if (STREQ(cmd, "deposit")) {
    ...
    else if ((money + GET_BANK(ch) > get_max_level(ch) * 40000) &&
             (get_max_level(ch) < 40)) {
      send_to_char
        ("I'm sorry, Regulations only allow us to ensure 40000 coins per level.\n\r",
         ch);
      return (TRUE);
    }
    else {
...
int pray_for_items(struct char_data *ch, const char *cmd, char *arg,
  ...
  if (!strcmp(arg, "one life")) {
    if (get_max_level(ch) >= LOW_IMMORTAL) {
      ...
      send_to_char
        ("Odin screams, 'you loser! Get back to work on your area!'\nr", ch);
      return (TRUE);
    }
...
int jugglernaut(struct char_data *ch, const char *cmd, char *UNUSED(arg),
  ...
  if (GET_POS(ch) == POSITION_STANDING) {
    ...
    if (random() % 6) {
      if (random() % 2) {
        act("$n tosses $p high into the air and catches it.", TRUE, ch,
            tmp_obj, NULL, TO_ROOM);
      }
      else {
...
int magic_user2(struct char_data *ch, const char *cmd, char *UNUSED(arg),
  ...
  if ((vict != ch->specials.fighting) && (lspell > 19)
    ...
    cast_fireshield(get_max_level(ch), ch, "", SPELL_TYPE_SPELL, ch, 0);
    return TRUE;
  }
  ...
  case 4:
    ...
    cast_chill_touch(get_max_level(ch), ch, "", SPELL_TYPE_SPELL, vict, 0);
    break;
  case 5:
  ...
  case 9:
    ...
    cast_fear(get_max_level(ch), ch, "", SPELL_TYPE_SPELL, vict, 0);
    break;
  case 10:
  case 11:
    act("$n utters the words 'Lightning Bolt'.", 1, ch, 0, 0, TO_ROOM);
    cast_lightning_bolt(get_max_level(ch), ch, "", SPELL_TYPE_SPELL, vict, 0);
    break;
  case 12:
  case 13:
...
void throw_char(struct char_data *ch, struct char_data *v, int dir) {
  ...
  if (rp && rp->dir_option[dir] &&
      rp->dir_option[dir]->to_room && (EXIT(v, dir)->to_room != NOWHERE)) {
    if (v->specials.fighting) {
      stop_fighting(v);
    }
  ...
  if (AWAKE(ch)) {
    if ((targ = find_an_attacker(ch)) != NULL) {
      ...
      if (!saves_spell(targ, SAVING_PARA)) {
        ...
        act("$N barely misses being swallowed whole!", FALSE, ch, 0, targ,
            TO_NOTVICT);
        act("You barely avoid being swallowed whole!", FALSE, ch, 0, targ,
            TO_VICT);
      }
    }
  }
...
int affect_status(struct memory *mem, struct char_data *UNUSED(ch),
                  struct char_data *t, int aff_status) {
  int i;
...
int trapper(struct char_data *ch, const char *cmd, char *UNUSED(arg),
  ...
  if (!ch->specials.fighting) {
    ...
    if (get_max_level(ch->specials.fighting) > MAX_MORT)
      return (FALSE);
...
#define DEITY_NAME "golgar"
  ...
  if (ch->specials.fighting) {
    if (number(0, 3) == 0) {
      for (tch = real_roomp(ch->in_room)->people; tch; tch = tch->next_in_room)
        if ((!IS_NPC(tch)) && (get_max_level(tch) > 20) && CAN_SEE(ch, tch)) {
...
int ghostsoldier(struct char_data *ch, const char *cmd, char *UNUSED(arg),
  ...
  if (good) {
    if (!check_soundproof(ch))
      act("$N attacks you with an unholy scream!", FALSE, good, 0, ch,
          TO_CHAR);
    hit(ch, good, TYPE_UNDEFINED);
    return (TRUE);
  }
...
#define Med_Chambers      21324
  ...
  case Valik_Qfour:
    ...
    break;
  default:
    log_msg("Ack! Foo! Heimdall screws up!");
    return (FALSE);
  }
}
...
#define Necklace 21122
  ...
  if (!cmd && (ch->generic == -1))
    ...
    if (!(pass = fopen(RHYODIN_FILE, "r"))) {
      log_msg("Rhyodin access file unreadable or non-existant");
      ch->generic = -1;
      return (FALSE);
    }
      ...
      if (obj_index[obj->item_number].virtual == Necklace) {
        ...
        if (!IS_NPC(ch)) {
          if (!(pass = fopen(RHYODIN_FILE, "a"))) {
            log_msg
              ("Couldn't open file for writing permanent Rhyodin passlist.");
            return (FALSE);
          }
