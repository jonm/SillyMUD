...
void do_inset(struct char_data *ch, char *argument,
  ...
  if ((ITEM_TYPE(sword) == ITEM_WEAPON) && (ITEM_TYPE(gem) == ITEM_TREASURE)) {
    ok = TRUE;
  }
...
void do_track(struct char_data *ch, char *argument,
  ...
  if (ch->specials.hunting) {
    ...
    if (GET_RACE(ch->specials.hunting) == RACE_VEGMAN &&
        real_roomp(ch->specials.hunting->in_room)->sector_type !=
        SECT_FOREST) {
      send_to_char("You are unable to find traces of one.\n\r", ch);
      ch->specials.hunting = 0;
      return;
...
#define CAULDRON 1531
    ...
    if (potion->obj_flags.value[j] >= 1) {
      for (i = 0; i < MIN_LEVEL_NUM; i++)
        if (skill_info[potion->obj_flags.value[j]].min_level[i] < LOW_IMMORTAL)
          max = MAX(max, skill_info[potion->obj_flags.value[j]].min_level[i]);
    }
  }
  ...
  if (ingredients != i || match != 0) {
    ...
    act
      ("You stood too close to the cauldron.  A drop of your sweat just fell in...",
       FALSE, ch, 0, 0, TO_CHAR);
    act("The cauldron overflows!  Everything is ruined!", FALSE, ch, 0, 0,
        TO_ROOM);
    act
      ("Next time pay more attention to your master... You just ruined this batch!",
       FALSE, ch, 0, 0, TO_CHAR);
  }
  else {
    potion = read_object(BrewList[which_potion].object[5], VIRTUAL);
