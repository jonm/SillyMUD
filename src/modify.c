...
void do_edit(struct char_data *ch, char *arg,
  ...
  case 3:
    ...
    if ((r_flags < 0) || (s_type < 0) || (s_type > 11)) {
      ...
      send_to_char
        ("flags must be 0 or positive, and sectors must be from 0 to 11\n\r",
         ch);
...
void check_reboot() {
  ...
  if ((t_info->tm_hour + 1) == REBOOT_AT && t_info->tm_min > 30)
    if ((boot = fopen("./reboot", "r")) != NULL) {
      if (t_info->tm_min > 50) {
        log_msg("Reboot exists.");
        fread(&dummy, sizeof(dummy), 1, boot);
        if (!feof(boot)) {      /* the file is nonepty */
          log_msg("Reboot is nonempty.");
          if (system("./reboot")) {
            log_msg("Reboot script terminated abnormally");
