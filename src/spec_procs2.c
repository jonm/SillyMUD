...
int druid_protector(struct char_data *ch, const char *cmd, char *UNUSED(arg),
                    struct char_data *UNUSED(mob), int UNUSED(type)) {
  ...
  if (cmd) {
    ...
    if (dir != MOVE_DIR_INVALID && IS_PC(ch)) {
      if (b) {
        ...
        if ((ch->in_room == Bandits_Path) && (dir == MOVE_DIR_NORTH)) {
...
int summoner(struct char_data *ch, const char *cmd, char *UNUSED(arg),
             struct char_data *UNUSED(mob), int UNUSED(type)) {
  extern struct descriptor_data *descriptor_list;
  ...
  if (GET_HIT(ch) > ((GET_MAX_HIT(ch) * 3) / 4)) {
    ...
    if (targ) {
      ...
      if (targ->in_room == ch->in_room) {
        if (num_charmed_followers_in_room(ch) > 0) {
...
int monk(struct char_data *ch, const char *cmd, char *UNUSED(arg),
         struct char_data *UNUSED(mob), int UNUSED(type)) {
...
int magic_user(struct char_data *ch, const char *cmd, char *arg,
...
int cleric(struct char_data *ch, const char *cmd, char *UNUSED(arg),
           struct char_data *UNUSED(mob), int UNUSED(type)) {
  struct char_data *vict;
...
int teacher(struct char_data *ch, const char *cmd, char *arg,
  ...
  if (STREQ(cmd, "practise") || STREQ(cmd, "practice")) {
    if (!arg || (strlen(arg) == 0)) {
    ...
    if (!charge) {
      if (!IS_SET(ch->player.class, skill_info[number].class_use)) {
        send_to_char
          ("'You do not possess the necessary training for this skill.'\n\r",
           ch);
        return (TRUE);
      }
    }
    ...
    if ((number == SKILL_DUAL_WIELD) && (ch->skills[SKILL_DODGE].learned < 80)
      ...
      send_to_char
        ("'Your dodge or backstab skill must be at least rank good.'\n\r", ch);
      return (TRUE);
    }
    ...
    for (i = 0; i < MAX_RACE_DENY; i++) {
      if (GET_RACE(ch) == skill_info[number].race_deny[i]) {
        send_to_char
          ("'Those of your race are unable to learn this skill.'\n\r", ch);
        return (TRUE);
      }
    }
    ...
    percent = ch->skills[number].learned + int_app[(int)GET_INT(ch)].learn;
...
int samah(struct char_data *ch, const char *cmd, char *arg,
          struct char_data *UNUSED(mob), int UNUSED(type)) {
  char *p, buf[256];
  ...
  if (cmd) {
    ...
    if (STREQ(cmd, "recite")) {
      ...
      if (strncmp("recall", p, strlen(p)) == 0) {
        act("$n says 'And just where do you think you're going, Mensch?",
      ...
      if (strncmp("'word of recall'", p, strlen(p) - 1) == 0) {
        ...
        act
          ("$n says 'Do you think you can astral walk in and out of here like the ",
           FALSE, Sammy, 0, 0, TO_ROOM);
        ...
        act("$n says 'I like you the way you are...Mensch.", FALSE, Sammy, 0,
            0, TO_ROOM);
        return (TRUE);
      }
    }
    else if (STREQ(cmd, "say") || STREQ(cmd, "'")) {
      act("$n glares at you", FALSE, Sammy, 0, ch, TO_VICT);
      act("$n glares at $N", FALSE, Sammy, 0, ch, TO_NOTVICT);
    ...
    else if (STREQ(cmd, "emote") || STREQ(cmd, ":") || STREQ(cmd, ",")) {
      act("$n glares at you", FALSE, Sammy, 0, ch, TO_VICT);
      act("$n glares at $N", FALSE, Sammy, 0, ch, TO_NOTVICT);
    ...
    else if (STREQ(cmd, "tell") || STREQ(cmd, "shout") || STREQ(cmd, "whisper")) {
      act("$n glares at you", FALSE, Sammy, 0, ch, TO_VICT);
      act("$n glares at $N", FALSE, Sammy, 0, ch, TO_NOTVICT);
      ...
      act("$n glares at $N", FALSE, Sammy, 0, ch, TO_NOTVICT);
      act
        ("$n says 'Your ignorance is too immense to be rectified at this time. Mensch.'",
         FALSE, Sammy, 0, 0, TO_ROOM);
      return (TRUE);
    }
    else if (STREQ(cmd, "order") || STREQ(cmd, "force")) {
      act("$n glares at you", FALSE, Sammy, 0, ch, TO_VICT);
      act("$n glares at $N", FALSE, Sammy, 0, ch, TO_NOTVICT);
    ...
    else if (STREQ(cmd, "flee")) {
      act("$n glares at you", FALSE, Sammy, 0, ch, TO_VICT);
      act("$n glares at $N", FALSE, Sammy, 0, ch, TO_NOTVICT);
      act("$n says 'Cease this cowardly behavior, Mensch'", FALSE, Sammy, 0,
          ch, TO_ROOM);
      return (TRUE);
    }
      ...
      act("$n glares at you", FALSE, Sammy, 0, ch, TO_VICT);
      act("$n glares at $N", FALSE, Sammy, 0, ch, TO_NOTVICT);
      act("$n says 'Pay attention when I am speaking, Mensch'", FALSE, Sammy,
          0, ch, TO_ROOM);
      return (TRUE);
    }
  }
...
int bitter_blade(struct char_data *ch, const char *cmd, char *arg,
  ...
  for (obj = real_roomp(ch->in_room)->contents; obj; obj = obj->next_content) {
    if (obj_index[obj->item_number].func == bitter_blade) {
      /* I am on the floor */
      ...
      if (lowjoe) {
        if (CAN_GET_OBJ(lowjoe, obj)) {
          ...
          act("A black blade jumps from the floor and leaps in to $n's hands!",
              FALSE, lowjoe, 0, 0, TO_ROOM);
          if (!ego_blade_save(lowjoe)) {
    ...
    for (obj = holder->carrying; obj; obj = obj->next_content) {
      if ((obj_index[obj->item_number].func) &&
        ...
        if (holder->equipment[WIELD]) {
          if ((!ego_blade_save(holder)) && (!ego_blade_save(holder))) {
            send_to_char
              ("The black blade gets pissed off that you are wielding another weapon!!\n\r",
               holder);
        ...
        if (affected_by_spell(holder, SPELL_CHARM_PERSON)) {
          ...
          send_to_char
            ("Due to the black blade, you feel less enthused about your master.\n\r",
             holder);
        }
      }
    }
      ...
      if ((obj_index[holder->equipment[WIELD]->item_number].func)
        ...
        if (holder->specials.fighting) {
          send_to_char("The black blade almost sings in your hand!!\n\r",
                       holder);
          act("You can hear $n's black blade almost sing with joy!", FALSE,
              holder, 0, 0, TO_ROOM);
          if ((holder == ch) && STREQ(cmd, "flee")) {
            if (ego_blade_save(ch) && ego_blade_save(ch)) {
              send_to_char
                ("You can feel the black blade attempt to stay in the fight!\n\r",
                 ch);
              return (FALSE);
            }
            else {
              send_to_char
                ("The black blade laughs at your attempt to flee from a fight!!\n\r",
                 ch);
              ...
              send_to_char("The black blade twists around and smacks you!\n\r",
                           ch);
              act
                ("Wow! $n's black blade just whipped around and smacked $m one!",
        ...
        if (STREQ(cmd, "remove") && (holder == ch)) {
          one_argument(arg, arg1);
          if (strcmp(arg1, "all") == 0) {
            if (!ego_blade_save(ch)) {
              send_to_char
                ("The black blade laughs at your attempt to remove it!\n\r",
                 ch);
              send_to_char("The black blade gives you a little warning...\n\r",
                           ch);
              send_to_char
                ("The black blade twists around and smacks you hard!\n\r", ch);
              act
                ("Wow! $n's black blade just whipped around and smacked $m one!",
                 FALSE, ch, 0, 0, TO_ROOM);
              GET_HIT(ch) -= 25;
              ...
              send_to_char
                ("You can feel the black blade attempt to stay wielded!\n\r",
                 ch);
              return (FALSE);
            }
          }
              ...
              if (!ego_blade_save(ch)) {
                send_to_char
                  ("The black blade laughs at your attempt to remove it!\n\r",
                   ch);
                send_to_char
                  ("The black blade gives you a little warning...\n\r", ch);
                send_to_char
                  ("The black blade twists around and smacks you hard!\n\r",
                   ch);
                act
        ...
        if (lowjoe) {
          if (!ego_blade_save(holder)) {
            ...
            act("A black blade jumps for $n's neck!", FALSE, lowjoe, 0, 0,
                TO_ROOM);
        ...
        if (STREQ(cmd, "hit") && (holder == ch)) {
          send_to_char("The black blade almost sings in your hand!!\n\r", ch);
          act("You can hear $n's black blade almost sing with joy!", FALSE, ch,
              0, 0, TO_ROOM);
          return (FALSE);
        }
      }
...
int creeping_death(struct char_data *ch, const char *cmd, char *UNUSED(arg),
                   struct char_data *UNUSED(mob), int UNUSED(type)) {
...
int generic_cityguardHateUndead(struct char_data *ch, const char *cmd, char *arg,
  ...
  if (ch->specials.fighting) {
    ...
    if (!check_soundproof(ch)) {
      ...
      if (ch->specials.fighting)
        call_for_guard(ch, ch->specials.fighting, 3, type);
  ...
  for (tch = real_roomp(ch->in_room)->people; tch; tch = tch->next_in_room) {
    if ((IS_NPC(tch)) && (is_undead(tch)) && CAN_SEE(ch, tch)) {
      max_evil = -1000;
      evil = tch;
      if (!check_soundproof(ch))
...
void druid_mob(struct char_data *ch) {
  ...
  ch->player.long_descr =
    (char *)strdup("A huge lion stands here, his tail twitches menacingly");
  GET_HIT(ch) = MIN(GET_MAX_HIT(ch), GET_HIT(ch));
  ch->mult_att = 3;
...
int druid_challenger(struct char_data *ch, const char *cmd, char *UNUSED(arg),
                     struct char_data *UNUSED(mob), int UNUSED(type)) {
  struct room_data *rp;
  int level;
  struct char_data *vict;
  ...
  if (!ch->specials.fighting) {
    ...
    if (IS_SET(rp->room_flags, NO_SUM) || IS_SET(rp->room_flags, TUNNEL) ||
      ...
      druid_attack_spells(ch, vict, level);
    }
...
int monk_challenger(struct char_data *ch, const char *cmd, char *UNUSED(arg),
                    struct char_data *UNUSED(mob), int UNUSED(type)) {
  struct char_data *vict;
...
int druid_challenge_prep_room(struct char_data *ch, const char *cmd,
  ...
  if (STREQ(cmd, "nod")) {
    ...
    mob =
      read_mobile(DRUID_MOB + GET_LEVEL(ch, DRUID_LEVEL_IND) - 10, VIRTUAL);
    if (!mob) {
...
int portal(struct char_data *ch, const char *cmd, char *arg, struct obj_data *obj,
  ...
  if (type == PULSE_COMMAND) {
    ...
    if (!
        (port =
         get_obj_in_list_vis(ch, obj_name,
    ...
    act("You enter $p, and you are transported elsewhere", FALSE, ch, port, 0,
        TO_CHAR);
    char_from_room(ch);
...
int scraps(struct char_data *UNUSED(ch), const char * UNUSED(cmd),
           char *UNUSED(arg), struct obj_data *obj, int type) {
...
int attack_rats(struct char_data *ch, const char * UNUSED(cmd), char *UNUSED(arg),
...
int dragon_hunter_leader(struct char_data *ch, const char * UNUSED(cmd),
...
int hunting_mercenary(struct char_data *ch, const char * UNUSED(cmd), char *UNUSED(arg),
                      struct char_data *mob, int type) {
  int dir;
...
int slot_machine(struct char_data *ch, const char *cmd, char *UNUSED(arg),
  ...
  if ((i[0] == i[1]) && (i[1] == i[2])) {
    ...
    case 5:
      ...
      act("Sirens start sounding and lights start flashing everywhere!", FALSE,
          ch, 0, 0, TO_ROOM);
      break;
    }
  ...
  if (STREQ(cmd, "enter")) {
        ...
        if ((portal = get_char_room("color pool", ch->in_room)) != NULL) {
          ...
          if (j > 0 && j < 32000) {
            ...
            send_to_char("You attempt to enter the pool, and it gives.\n\r",
                         ch);
            send_to_char
              ("You press on further and the pool surrounds you, like some soft membrane.\n\r",
               ch);
            send_to_char
              ("There is a slight wrenching sensation, and then the color disappears.\n\r",
               ch);
            send_to_char("\n\r", ch);
    ...
    if (!number(0, 15)) {
      ...
      while (1) {
        ...
        if (going_to < 0 || going_to < ASTRAL_START || going_to > ASTRAL_END)
          continue;
...
int dwarven_miners(struct char_data *ch, const char * UNUSED(cmd), char *UNUSED(arg),
                   struct char_data *UNUSED(mob), int type) {
  if (type == PULSE_COMMAND)
    return (FALSE);
...
int real_rabbit(struct char_data *ch, const char *cmd, char *UNUSED(arg),
                struct char_data *UNUSED(mob), int UNUSED(type)) {
  struct char_data *i;
...
int real_fox(struct char_data *ch, const char *cmd, char *UNUSED(arg),
             struct char_data *UNUSED(mob), int UNUSED(type)) {
  struct char_data *i;
...
int antioch_grenade(struct char_data *UNUSED(ch), const char *cmd, char *arg,
