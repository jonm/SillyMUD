...
void do_look(struct char_data *ch, char *argument, int UNUSED(cmd)) {
  ...
  if (IS_AFFECTED(ch, AFF_BLIND))
    ...
    if (keyword_no != 8 && keyword_no != 9 && IS_AFFECTED2(ch, AFF2_SUN_BLIND)) {
      send_to_char
        ("The daylight is too bright, you can't see a damn thing!\n\r", ch);
    }
    else
      switch (keyword_no) {
        /* look <dir> */
        ...
        if (IS_AFFECTED2(ch, AFF2_SUN_BLIND)) {
          ...
          if (!IS_NPC(ch)) {
            ...
            if (IS_SET(ch->specials.act, ACT_HUNTING)
                || IS_SET(ch->specials.act, PLR_HUNTING)) {
              if (ch->specials.hunting) {
                res = track(ch, ch->specials.hunting);
                if (!res) {
                  ch->specials.hunting = 0;
  ...
  if (!IS_IMMORTAL(ch) || cmd == 234) {
    ...
    if (strlen(argument) == 0) {
      ...
      if (arg[0] == '-') {
        if (index(arg, '?') != NULL) {
          send_to_char
            ("[-]i=idle l=levels t=title h=hit/mana/move s=stats\n\r", ch);
          send_to_char
            ("[-]d=linkdead g=God o=Mort [1]Mage[2]Cleric[3]War[4]Thief[5]Druid[6]Monk\n\r",
             ch);
          send_to_char("--------\n\r", ch);
        }
        for (person = character_list; person; person = person->next) {
                ...
                for (l = 1; (size_t) l <= strlen(arg); l++) {
                  ...
                  case 'h':{
                      SPRINTF(tempbuf, "Hit:[%-3d] Mana:[%-3d] Move:[%-3d] ",
                              GET_HIT(person), GET_MANA(person),
                              GET_MOVE(person));
                      strcat(buf, tempbuf);
                      break;
                    }
                  case 's':{
                      SPRINTF(tempbuf, "[S:%-2d I:%-2d W:%-2d C:%-2d D:%-2d] ",
                              GET_STR(person), GET_INT(person),
                              GET_WIS(person), GET_CON(person),
                              GET_DEX(person));
                      strcat(buf, tempbuf);
                      break;
                    }
  ...
  if (!has_class(ch, CLASS_THIEF)) {    /* it had better be an intrinsic */
    if (GET_RACE(ch) == RACE_DWARF &&
        (GET_ITEM_TYPE(obj) != ITEM_WEAPON &&
         ...
         GET_ITEM_TYPE(obj) != ITEM_BOAT && GET_ITEM_TYPE(obj) != ITEM_WORN)) {
