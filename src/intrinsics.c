...
void do_changeform(struct char_data *ch, char *argument, const char * UNUSED(cmd)) {
  ...
  if (!*buf) {
    if (has_class(ch, CLASS_DRUID)) {
      ...
      send_to_char("     ", ch);
      for (mobn = 1;
           LAST_DRUID_MOB + 1 >= mobn && level >= DruidList[mobn].level;
           mobn++) {
        SPRINTF(buf, "%2d - %-15s ", DruidList[mobn].level,
                DruidList[mobn].name);
        if (!(mobn % 3))
          strcat(buf, "\n\r     ");
        send_to_char(buf, ch);
  ...
  if (has_class(ch, CLASS_DRUID)) {
    ...
    if (mob) {
      ...
      send_to_char
        ("Ack, the bat form is acting screwy, not bats! Bug an imp!\n\r", ch);
      return;
    }
  }
  else {
  ...
  if (*arg) {
    ...
    if (victim) {
      if (get_max_level(victim) >= LOW_IMMORTAL) {
        ...
        send_to_char
          ("Your master would be very unpleased if you tried that!\n\r", ch);
        return;
      }
      if ((!ch->specials.fighting)) {
        if ((!victim->specials.fighting)) {
          if (GET_POS(ch) >= POSITION_STANDING) {
            ...
            send_to_char
              ("Hey, why don't you try doing this when on your feet!\n\r", ch);
        }
        else
...
void mindflayer_attack(struct char_data *ch, struct char_data *v) {
  ...
  if (hit_or_miss(ch, v, calc_thaco(ch)) && get_max_level(v) < LOW_IMMORTAL) {
    ...
    act("$n's tentacles momentarily wrap themselves around your head!", FALSE,
        ch, 0, v, TO_VICT);
    send_to_char
      ("Blinding pain fills your head, and flashes of red dot your vision.\n\r",
       v);
...
void vampire_bite(struct char_data *ch, struct char_data *v) {
  ...
  if (v->specials.bitten) {     /* vampire's victim */
    ...
    if (chance > number(1, 100)) {
      ...
      act("$N limply struggles in $n's clutches.", FALSE, ch, 0, v,
          TO_NOTVICT);
      act("You struggle to free yourself of $n's grasp, but can't.", FALSE, ch,
          0, v, TO_VICT);
      to_bite = TRUE;
    }
  }
  ...
  if (to_bite) {
    ...
    if (GET_HIT(v) > 0) {
      ...
      send_to_char
        ("You cannot drain anything more from this lifeless husk.\n\r", ch);
    ...
    if (weather_info.sky < 2) { /* partly cloudy & sunny */
      if (!IS_SET(real_roomp(ch->in_room)->room_flags, INDOORS)) {
        if (!IS_SET(real_roomp(ch->in_room)->room_flags, DARK)) {
          ...
          if (msgs)
            send_to_char
              ("You wish the sun were caressing your limbs once again.\n\r",
               ch);
        }
      }
      else {                    /* we are indoors */
