  ...
  if (str_cmp(obj_name, "coins") && str_cmp(obj_name, "gold")) {
    ...
    if (AWAKE(victim) && (percent > ch->skills[SKILL_STEAL].learned)) {
      ...
      if (ch->skills[SKILL_STEAL].learned < 90) {
        act
          ("Even though you were caught, you realize your mistake and promise to remember.",
           FALSE, ch, 0, 0, TO_CHAR);
...
void do_recite(struct char_data *ch, char *argument, int UNUSED(cmd)) {
  ...
  act("You recite $p which bursts into butterflies.", FALSE, ch, scroll, 0,
      TO_CHAR);
...
void do_use(struct char_data *ch, char *argument, int UNUSED(cmd)) {
  ...
  if (stick->obj_flags.type_flag == ITEM_STAFF) {
    ...
    for (fol = ch->followers; fol; fol = fol->next)
      if (IS_SET(fol->follower->specials.act, ACT_FIGURINE)) {
        send_to_char
          ("The beast you attempted to summon will not come to this plane of existance.\n",
           ch);
        return;
      }
    ...
    if (GET_HIT(stick->link) < GET_MAX_HIT(stick->link) / 7) {
      send_to_char
        ("The beast refuses to step foot into this plane of existance until it has healed.\n\r",
         ch);
      return;
    }
    stick = unequip_char(ch, HOLD);
